...
class VideoView: UIView,WKNavigationDelegate,WKUIDelegate {
    ...
    func  setUp() {
        ...
        // config.mediaTypesRequiringUserActionForPlayback = .all ///把手动播放设置NO
...
//        wkWebView.loadFileURL(fileURL, allowingReadAccessTo: fileURL)
        ...
        //http://oss-hz.qianmi.com/x-site/test/d2cStore/A868907/view/adScreen/index/1517908799426.html
        wkWebView.load(URLRequest.init(url: URL.init(string: "http://oss-hz.qianmi.com/x-site/test/d2cStore/A868907/view/adScreen/index/1517908799426.html")!, cachePolicy: URLRequest.CachePolicy.returnCacheDataElseLoad, timeoutInterval: 20))
...
//
    ...
    func playVideo(){
        let script = "var videos = document.querySelectorAll(\"video\"); for (var i = videos.length - 1; i >= 0; i--) { var ivideo = videos[i]; ivideo.play(); };"
